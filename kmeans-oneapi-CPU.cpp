...
// Assigns each instance to the nearest cluster center (runs on CPU using SYCL)
void assign_cluster(buffer<float, 1>& instances_buf, buffer<float, 1>& centers_buf) {
    buffer results_buf(results);
    range<1> global_size(n_instances);
...
// Updates cluster centers based on assigned instances (runs on CPU using SYCL)
void update_centers(buffer<float, 1>& center_buf, int* c, buffer<float, 1>& instance_buf, int seed) {
    ...
    range<1> global_size(k * n_attributes);
    event e = q.submit([&](handler& h) {
